...
if (isset($_GET["nivel_org"])) {
	$eva = $bd->getEvaluadorGer($_GET["nivel_org"]);
	$gerencia = $eva["gerencia"];
?>
					...
					<div class="col-12">
						<div class="d-flex flex-row justify-content-between align-items-center">
							<h2 class="m-0 fw-bold">
								<?= ucfirst(mb_strtolower($gerencia, "UTF-8")) ?>
		...
		?>
			...
			if (isset($_SESSION['rol'])) {
				...
				foreach ($q as $q) {
					...
					elseif ($evaluador = $bd->getEvaluadorGer($_GET['nivel_org'])) {
						$evaluador = $evaluador['id'];
						$sin_evaluador = false;			
					}
					else {
