...
public function getLoginUsuario($nameuser, $pass)
        {
            try
            {
                /*$query="SELECT *
                        ...
                        ON u.id_perfil = p.id_perfil WHERE login = :nameuser AND pass = :pass AND estatus = '1';";*/
				...
				$query = "SELECT login, u.id_perfil, id_ccosto as id_nivel_org, emp.cedula as evaluador
						  ...
						  WHERE login = :nameuser AND pass = :pass AND estatus = '1';";
...
public function getEmpleadosxEval($nivel_org,$evaluador) {
        try
        {
      /*  echo  $query="SELECT id, cedula, nombre, apellido, id_nomina
		 		 FROM se_empleado
				 WHERE id_ccosto = ". $nivel_org ." and evaluado_por = '". $evaluador . "'";*/
...
public function getEmpleadosEvaluados($evaluador) {
        try
        {
         $query="SELECT emp.id, emp.id_nomina ,id_eval ,cedula, nombre,apellido,  car.descripcion as cargo, (SELECT concat_ws(' ', nombre, apellido) from se_empleado where cedula = (SELECT evaluado_por FROM se_empleado where cedula = ". $evaluador .")) to_send ,eva.puntaje
					  FROM se_empleado as emp 
...
public function getResultadoxNivel($condicion,$nivel_org,$evaluador,$sinUni) {
    try
    {
     ...
     or  (". $condicion ."  and se_empleado.id_ccosto = ". $nivel_org .")";   //. and evaluado_por = ".$evaluador;*/
     if($sinUni){
     $query = "select count(puntaje) as puntaje from se_evaluacion 
            inner join se_empleado
            on se_evaluacion.id_empleado = se_empleado.id
...
public function getEvaluadorGer($nivel_org) {
    try
    {
     /*$query="SELECT cedula, bd_nivel_org.gerencia_gral.descripcion as gerencia FROM bd_eval_personal.se_empleado
     inner join bd_nivel_org.nivel_org 
     on id_ccosto = bd_nivel_org.nivel_org.id
     inner join bd_nivel_org.gerencia_gral
     on bd_nivel_org.nivel_org.id_ger_gral = bd_nivel_org.gerencia_gral.id_ger_gral
     WHERE id_ccosto = ". $nivel_org ." and es_evaluador = 1";*/
...
/*public function getDatosEmpleadosEditar($id_empleados) {
        try
        {
...
public function getDatosEmpleadosEditar($id) {
        try
        {
         /*$query=" SELECT id_datos_personas, cedula, nombres, apellidos, gerencia, unidad, cargo, ubi_fisica, sc_datos_personas.id_vehiculo, estatus, marca, modelo, placa, color, tipo_combus from sc_datos_personas inner join sc_vehiculos  where sc_datos_personas.id_vehiculo = sc_vehiculos.id_vehiculo and id_datos_personas = '$id' ";*/
...
/*************************************************************************************************************/
     ...
     public function insertEvaluacion($data)
    {
        ...
        {//id_eval 	fecha 	puntaje 	estatus 	id_empleado 	
            $query = "INSERT INTO se_evaluacion ( fecha, puntaje, estatus, id_empleado )
			VALUES (:fecha, :puntaje, :estatus, :id_empleado)";
...
/*************************************************************************************************************/
    ...
    public function deleteEvaluacion($query)
    {
        try
