...
where puntaje <= 8 and se_empleado.id_ccosto = 511
			...
			*/
	//echo "ddd: " . $_SESSION["id_perfil"];
	if (isset($_GET["nivel_org"])) {
		$evaluador = $bd->getEvaluadorDiv($_GET["nivel_org"]);
		$division = $evaluador["division"];
	...
	?>
			...
			<?php
			//hay que recibir el perfil que se quiere gestionar en esta pagina y el evaluador
	   ...
	   // echo "el centinela es: ". $cent;
		...
		case 2: 
			...
			$q = $bd->getDivisiones($_SESSION["evaluador"]); 
		  break;
		 case 3: 
			$href = "detalle_coordinacion.php?nivel_org=";
			$dep = $bd->getDep($_GET["nivel_org"]);
			if ($dep["id_dep"] != 0){		 
				$q = $bd->geDepartamentos($_SESSION["evaluador"], 0); 
			}else{
				$q = $bd->getCoordinaciones($evaluador["cedula"], 0); 
			}
		...
		//  break; 
		 default:
		  $msj = "";
				...
				//echo "</BR>: ".$_GET["nivel_org"];


				$div = $bd->getDiv($_GET["nivel_org"]);
				if ($div) {



					//$dep = $bd->getDep($div["id_div"]);
					$q = null;
					if ($array_dep = $bd->getDepartamentos($evaluador["cedula"], 1)) {
						//; 
						//echo "wwwwwwwwwww";
						$q = $array_dep;
				...
				if ($q) {
					foreach ($q as $q) {
						...
						if ($eval) {
							$evaluador = $eval['cedula']; //$href[$i] = "detalle_departamento.php?nivel_org=";
							$sinUni = 1;
						} else {
							$eval =  $bd->getEvaluadorDiv($_GET["nivel_org"]); //getEvaluadorDiv($nivel_org)
							$evaluador = $eval['cedula'];
							$sinUni = 0;
						}	//$evaluador = '886557';	
						/*  $dep = $bd->getDep($div["id_div"]);*/
						//echo  "</br>".substr($q['des'],0,3);
						...
						<div class="row">
							...
							<div class="col-12">
								<div class="card">
									...
									<div class="card-header" >
										<h3 class="card-title">Resumen de evaluaciones por Departamento/Coordinación</h3>
...
<?php
			...
			$division = $bd->getDivisiones($evaluador["cedula"], 0);
?>
...
<section class="content">
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-12">
				<div class="card-header">
					<h3 class="card-title"><?php echo $division["des"]; ?></h3>
				</div>
			</div>
